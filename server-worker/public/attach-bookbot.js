...
async function requestBio() {
	...
	if (!firstName) {
		setBioError('First name required.')
		return
	}
	...
	url.searchParams.set('type', 'bio')
	url.searchParams.set('author', fullName)

	const source = new EventSource(url.toString())

	let firstPacket = true
	const priorText = bio.value

	const { promise, resolve } = Promise.withResolvers()

	const close = () => {
		source.close()
		if (bio.value.toLocaleLowerCase().includes('invalid prompt')) {
			bio.value = priorText
			setBioError('This author was not found.')
		}
		resolve()
	}
...
function setBioError(message) {
	document.getElementById('bio-id').classList.add('is-invalid')
	document.getElementById('bio-errors').textContent = message
}

function clearBioError() {
	document.getElementById('bio-id').classList.remove('is-invalid')
	document.getElementById('bio-errors').textContent = ''
}
...
export function attachSummarizer() {
	...
	if (summaryContainer) {
		const button = makeButton('Suggest summary')
	...
	if (bioContainer) {
		const button = makeButton('Suggest biography')
		...
		bioContainer.append(button)

		document
			.getElementById('first-id')
			?.addEventListener('input', clearBioError)
		document.getElementById('last-id')?.addEventListener('input', clearBioError)
		document.getElementById('bio-id')?.addEventListener('input', clearBioError)
	}
}

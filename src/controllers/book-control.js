...
const preventTitleCollision = body('title')
	...
	.custom(async (value) => {
		...
		if (priorEntry) {
			throw new Error(`Book already recorded: #` + priorEntry[0].book_id)
		}
	})

const onlySelfTitleCollision = body('title', 'Title already in catalog')
	.trim()
	.custom(async (value, { req }) => {
		const result = await books.find({ title: value })
		if (result && String(req.params.id) !== String(result[0].book_id)) {
...
exports.book_list = [
	...
	async (req, res) => {
		...
		if (search) {
			const matches = await trigramTitleQuery(req.client, search)
...
exports.book_create_post = [
	...
	async (req, res) => {
		...
		suggestBook(
			result[0].title,
			(await authors.find('full_name', { author_id: result[0].author_id }))[0]
				.full_name,
			result[0].book_id
		)
...
async function suggestRecent(workKey, book_id) {
	...
	try {
		...
		if (!Array.isArray(response.data.covers)) {
			log(
				`Rejecting suggestion ` + `(no covers available for ${workKey})`,
				pink
			)
			return
		}
